...
class Uniswap:
    def __init__(self, wallet_address, private_key, provider, web3):
        ...
        self.router = self.w3.eth.contract(
            address=self.router_address, abi=UNIVERSAL_ROUTER_ABI
        )
    ...
    def approve_permit2(self, token_address, amount):
        ...
        contract_function = token_contract.functions.approve(
            self.permit2.address, max_approval
        )
    ...
    def create_permit_signature(self, token_address):
        ...
        permit2_contract = self.w3.eth.contract(
            address=self.permit2.address, abi=PERMIT2_ABI
        )

        p2_amount, p2_expiration, p2_nonce = permit2_contract.functions.allowance(
            self.wallet_address, token_address, self.router_address
        ).call()

        print(
            "p2_amount, p2_expiration, p2_nonce: ", p2_amount, p2_expiration, p2_nonce
        )
    ...
    def check_permit2_allowance(self, token_address):
        ...
        permit2_allowance = token_contract.functions.allowance(
            self.wallet_address, self.permit2.address
        ).call()

        print(f"Current Permit2 allowance: {permit2_allowance}")

        # Check if allowance is effectively infinite (very large number)
        LARGE_APPROVAL_THRESHOLD = (
            2**200
        )  # Any number larger than this is considered "infinite"

        return permit2_allowance > LARGE_APPROVAL_THRESHOLD

    def make_trade(
        self, from_token, to_token, amount, fee, slippage, pool_version="v3"
    ):
        """
        ...
        has_permit2_allowance = self.check_permit2_allowance(token0)
        if not has_permit2_allowance:
            print("Permit2 approval needed. Initiating approval...")
            approval_success = self.approve_permit2(token0, amount)
        ...
        permit_data, signed_message = self.create_permit_signature(token0)
        ...
        decimals_in = self.get_token_decimals(token0)
        ...
        if pool_version.lower() == "v3":
            ...
            is_native_input = (
                token0.lower() == "0x0000000000000000000000000000000000000000"
                # from_token.lower() == "0x0000000000000000000000000000000000000000"
            )
            ...
            v4_transaction = (
                ...
                .take_all(token1 if zeroForOne else token0, Wei(0))
                .settle_all(token0 if zeroForOne else token1, amount_in_wei)
                .build_v4_swap()
                .build_transaction(
                    self.account.address,
                    amount_in_wei if is_native_input else Wei(0),
        ...
        tx = {
            ...
            "data": encoded_data,
            "value": amount_in_wei
            if token0.lower() == "0x0000000000000000000000000000000000000000"
            else 0,
            "nonce": self.w3.eth.get_transaction_count(self.account.address),
            "gas": gas_params[
                "estimated_total_wei"
    ...
    def cancel_transaction(self, stuck_nonce):
        ...
        try:
            ...
            new_max_fee_per_gas = max(
                base_fee * base_multiplier + priority_fee * 3, Web3.to_wei(0.1, "gwei")
            )
            new_max_priority_fee = max(
                priority_fee * priority_multiplier, Web3.to_wei(0.005, "gwei")
            )
            ...
            print(
                f"🔹 Max Fee per Gas: {Web3.from_wei(new_max_fee_per_gas, 'gwei')} Gwei"
            )
            print(
                f"🔹 Max Priority Fee per Gas: {Web3.from_wei(new_max_priority_fee, 'gwei')} Gwei"
            )
            ...
            if balance < total_gas_wei:
                ...
                print(
                    f"Need {(total_gas_eth - Web3.from_wei(balance), 'ether')} more ETH"
                )
                return
            ...
            print(
                f"New Priority Fee: {Web3.from_wei(new_max_priority_fee, 'gwei')} gwei"
            )
    ...
    def calculate_gas_parameters(self, estimated_gas_limit=21000):
        ...
        try:
            ...
            new_max_fee_per_gas = int(
                base_fee * base_multiplier + priority_fee * priority_multiplier
            )
    ...
    def check_balance(self, amount, decimals_in, token_contract):
